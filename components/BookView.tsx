...
const buildBook = (levels: Level[], bookTitle?: string): { book: BookContent; pagesPerChapter: number } => {
    ...
    const book: BookContent = { title: bookTitle || 'Gesture Book Reader', author: bookTitle ? 'Your Document' : 'SAP', pages };
...
const BookView: React.FC<BookViewProps> = ({ levels: providedLevels, pointer, isPinching, onChapterQuiz, bookTitle }) => {
    ...
    const { book } = useMemo(() => {
        if (levels.length === 0) {
            return {
                book: {
                    title: 'Welcome', author: '', pages: [{
                        pageNumber: 1,
                        title: 'Welcome',
                        content: ['Please upload a PDF to begin reading.'],
                        type: 'text'
                    }]
                }
            };
